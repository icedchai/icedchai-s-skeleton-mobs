...
public class EntityInit {
    public static final EntityType<SkeletonKnightEntity> SKNIGHT = Registry.register(
            ...
            FabricEntityTypeBuilder.create(SpawnGroup.MONSTER, SkeletonKnightEntity::new).dimensions(EntityDimensions.fixed(0.9f, 0.9f)).build()
    );
    public static final EntityType<WitherArcherEntity> WARCHER = Registry.register(
            Registry.ENTITY_TYPE,
            new Identifier(MODID, "wither_archer"),
            FabricEntityTypeBuilder.create(SpawnGroup.MONSTER, WitherArcherEntity::new).fireImmune().dimensions(EntityDimensions.fixed(0.9f, 0.9f)).build()
    );
